...
* Description   : Decodes the push buttons direction
* Output        : None
...
u8  ButtonState(void)
{
  ...
  if (GPIO_ReadBit( GPIO7,  GPIO_Pin_5)==0) return(RIGHT);
  if (GPIO_ReadBit( GPIO7,  GPIO_Pin_6)==0) return(LEFT);
  if (GPIO_ReadBit( GPIO7,  GPIO_Pin_3)==1) return(UP);
  if (GPIO_ReadBit( GPIO7,  GPIO_Pin_4)==1) return(DOWN);
  else return(0);
}
