...
;--- STR9X BOOT specific definitions
BOOTADDR  EQU 0x00000000  ;Boot Block Address
NBOOTADDR EQU 0x00080000  ;Not Boot Block Address
#ifdef Remap_Bank_1
BANK_1_ADDR EQU BOOTADDR
BOOTSIZE    EQU 1   ;BLOCK1SIZE KB = 2^BOOTSIZE*32 KB*/
NBOOTSIZE   EQU 6   ;BLOCK0SIZE KB = 2^NBOOTSIZE*8 KB
#else
BANK_1_ADDR EQU NBOOTADDR
BOOTSIZE    EQU 4   ;BLOCK0SIZE KB = 2^BOOTSIZE*32 KB
NBOOTSIZE   EQU 2   ;BLOCK1SIZE KB = 2^NBOOTSIZE*8 KB
...
;     when the bank 0 is the boot bank, then enable the Bank 1.
        ...
        LDR     R7, = BOOTSIZE                ; BOOT BANK Size
        STR     R7, [R6, #FMI_BBSR_OFST]      ;  
        
        LDR     R7, = NBOOTSIZE               ; NON BOOT BANK Size
        STR     R7, [R6, #FMI_NBBSR_OFST]     ; 

        LDR     R7, = BOOTADDR>>2             ; BOOT BANK Address = 0x0 
        STR     R7, [R6, #FMI_BBADR_OFST]

        LDR     R7, = NBOOTADDR>>2            ; NON BOOT BANK Address = 0x80000
...
; --- Now enter the C code
       ...
       SECTION .flash_conf:CODE:NOROOT(2)
       CODE32              
flash_conf
