...
volatile UserFunc_t UsbUserFun [UsbLastEvent] =
{
  // Ep 0 Out
  NULL,
  // Ep 0 In
...
Int32U State;
  ...
  // It will loot until the desired state and the EP's state are equal
...
{
#if USB_CONN_CTRL > 0
  GPIO_WriteBit(GPIO7,GPIO_Pin_0,Conn?Bit_RESET:Bit_SET);
#endif // USB_CONN_CTRL > 0
}
...
Int32U  ActSize, Temp, Count, CurrSize, SizeHold;
  ...
  *pCount = 0;
  ...
  if(EpState.STAT_TX == EP_DISABLED)
  {
