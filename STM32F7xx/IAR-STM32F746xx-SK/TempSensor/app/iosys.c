...
{
int ch;
unsigned int status = COM1_BASE->ISR;

  if(status & UART_FLAG_RXNE)
  {
    ch = UartHandle.Instance->RDR;
    if(status & (UART_FLAG_ORE | UART_FLAG_PE | UART_FLAG_FE) )
    {
...
{
  if (HAL_UART_Transmit(&UartHandle, (uint8_t *)Buf, Bufsize, 0xFFFFFFFF) == HAL_OK)
    return Bufsize;
  else
    return 0;
}
