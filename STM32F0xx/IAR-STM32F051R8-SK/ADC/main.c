 ...
 * basic use of a LCD driver and an ADC.
 *  The example show position of the trimer in percents on the LCD display
 *
 *    $Revision: 103 $
 **************************************************************************/
...
int main(void)
{
 ...
 char buffer[9];
  ...
  while(1)
  {
    ...
    ADC_StartOfConversion(TRIM_ADC);
    while(!ADC_GetFlagStatus(TRIM_ADC, ADC_FLAG_EOC));
    temp = ADC_GetConversionValue(TRIM_ADC);
    ADC_ClearFlag(TRIM_ADC, ADC_FLAG_EOC);
    temp = (temp*100/(4096-1));
    sprintf(buffer,"TR:%3d% ",temp);
