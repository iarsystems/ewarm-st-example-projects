...
#define LD8	GPIO_PIN_12	
#define LD9	GPIO_PIN_13	
#define LD10	GPIO_PIN_14	
#define LD11	GPIO_PIN_15	
...
CAN_InitTypeDef CAN1Init = {0x0,CAN_BITRATE_100K};
	...
	GPIO_BitWrite(GPIO0,LD4,Bit_SET);//LED4;
	...
	while (!CAN_ReceiveMessage(CAN1, CAN1_RX_MSGOBJ, FALSE, &RxCan1Msg))
	{
        ...
        if((RxCan1Msg.IdType == CAN_STD_ID)&&(RxCan1Msg.Id == 0x123)&&(RxCan1Msg.Dlc == 4)
          ...
          //Received Msg OK
	GPIO_BitWrite(GPIO0,LD3,Bit_SET);//  LED3;
        } else {
          //Received Msg KO
        GPIO_BitWrite(GPIO0,LD4,Bit_RESET);//LED4 off;
        }
...
CANInit.CAN_Bitrate = CAN_BITRATE_500K;
	...
	GPIO_BitWrite(GPIO0,LD6,Bit_SET);//LED6;
	...
	while (!CAN_ReceiveMessage(CAN2, CAN2_RX_MSGOBJ, FALSE, &RxCan2Msg))
	{
        ...
        if((RxCan2Msg.IdType == CAN_STD_ID)&&(RxCan2Msg.Id == 0x123)&&(RxCan2Msg.Dlc == 4)
          ...
          //Received Msg OK
	GPIO_BitWrite(GPIO0,LD5,Bit_SET);//LED5;
        } else {
          //Received Msg KO
        GPIO_BitWrite(GPIO0,LD6,Bit_RESET);//LED6 off;
        }
...
CAN1Init.CAN_Bitrate = CAN_BITRATE_500K;
	...
	GPIO_BitWrite(GPIO0,LD9,Bit_SET);//LED9
...
CAN2Init.CAN_Bitrate = CAN_BITRATE_125K;
	...
	GPIO_BitWrite(GPIO0,LD11,Bit_SET);//LED11
...
int main()
{
	...
	GPIO_WordWrite(GPIO0,0x0000);//Leds off
