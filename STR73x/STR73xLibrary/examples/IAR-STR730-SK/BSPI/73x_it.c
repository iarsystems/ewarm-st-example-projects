...
u8 Index = 0;
extern u8 Tx_Buffer[];
extern u8 NbrDataToTransfer;
extern u8 Rx_Buffer[];
extern Status TransferStatus;
...
{
  if(Index < NbrDataToTransfer) /* the whole data are not yet transmitted */
  {
    BSPI_WordSend(BSPI1, Tx_Buffer[Index]);
    Index++;
  }	
}
...
void DMA0_IRQHandler (void)
{
  ...
  /* disable BSPI0 */
  BSPI_Cmd(BSPI0, DISABLE);

  /* clear DMA0 stream2 transfer end interrupt(SI2) flag */
  DMA_FlagClear(DMA0, DMA_FLAG_INT2);

  /* disable DMA access on AHB bus */
  DMA_AHBArbitrationConfig(DMA_Priority_Low);

/* check if the data received by the BSPI0 is correct ------------------------*/
  TransferStatus = Buffercmp(Tx_Buffer, Rx_Buffer, NbrDataToTransfer);

  /* TransferStatus = PASS, if the data received by the BSPI0 is correct */
  /* TransferStatus = FAIL, if the data received by the BSPI0 is corrupted */
