...
Int8U Data[3];
  ...
  RCC_AHB1PeriphClockCmd(TEMP_SENSOR_INT_CLK , ENABLE);

  // Init temperature sensor interrupt signal
  GPIO_InitStructure.GPIO_Pin = TEMP_SENSOR_INT_PIN;
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
  GPIO_Init(TEMP_SENSOR_INT_PORT, &GPIO_InitStructure);
...
Int16S Temp;
  ...
  if(Bit_SET == GPIO_ReadInputDataBit(TEMP_SENSOR_INT_PORT, TEMP_SENSOR_INT_PIN))
  {
    *pAlarm = FALSE;
  }
  else
