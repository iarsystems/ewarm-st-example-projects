...
STM32F407zg-SK, which you can use to evaluate the capabilities of the microcontroller
...
- Test folder:
The Test folder contains STM32F407ZG-SK peripheral tests that make up the core 
of the demonstration. These tests demonstrate some capabilities of the 
microcontroller.
  
- library folder
The STM32F4xx_StdPeriph_Driver folder contains all the subdirectories and files that make 
up the core of the STM32F4xxx Firmware library:
  - inc subfolder contains the firmware library header files
  - src subfolder contains the firmware library source files
  
- project folder
project folder contains project for the EWARM toolchain (V6.20) that 
compiles all the demonstration files.

Demonstration description 
==========================

1. After power ON(*), there would be the "STM32F407ZG-SK" message and IAR and ST logos 
displayed on LCD.
...
periph test sub-menu in which you can test STM32F407ZG IAR SK components:
    ...
    - Buzzer            : Generating Beep signal on the buzzer  .
    
    - Audio In/Out      : Play sounds through th headphone.
...
3. If you choose Communication Periph (pressing Joystick key (J)), you can test:
    ...
    - RS232_1          : Using the USART1 to communicate with the hyperterminal with interactive
                        messages on LCD.  
    
    - CAN loopback            : Performing a communication and data transfert between CAN1 an CAN2.  
...
How to use it
============= 
 - Open project at IAR-STM32F407ZG-SK\TEST\TEST.eww  
