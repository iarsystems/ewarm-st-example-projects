...
GPIO_TypeDef*  CAP_BUTTON_PORT[CAP_BUTTONn] = { CAP_BUT1_GPIO_PORT, CAP_BUT2_GPIO_PORT };

const uint16_t CAP_BUTTON_PIN[CAP_BUTTONn] = { CAP_BUT1_PIN, CAP_BUT2_PIN };

const uint32_t CAP_BUTTON_CLK[CAP_BUTTONn] = { CAP_BUT1_GPIO_CLK, CAP_BUT2_GPIO_CLK };

const uint16_t CAP_BUTTON_EXTI_LINE[CAP_BUTTONn]   = { CAP_BUT1_EXTI_LINE,
                                                       CAP_BUT2_EXTI_LINE };

const uint16_t CAP_BUTTON_PORT_SOURCE[CAP_BUTTONn] = { CAP_BUT1_EXTI_PORT_SOURCE,
                                                       CAP_BUT2_EXTI_PORT_SOURCE };

const uint16_t CAP_BUTTON_PIN_SOURCE[CAP_BUTTONn] = { CAP_BUT1_EXTI_PIN_SOURCE,
                                                      CAP_BUT2_EXTI_PIN_SOURCE };
                                             
const uint16_t CAP_BUTTON_IRQn[CAP_BUTTONn] = { CAP_BUT1_EXTI_IRQn,
                                                CAP_BUT2_EXTI_IRQn };
...
/*************************************************************************
 * Function Name: STM_CapBut_Discharge
 * Parameters: CapButton_TypeDef CapBut
 *
 * Return: none
 *
 * Description: Discharges the given capacitive sensor button.
 *
 *************************************************************************/
void STM_CapBut_Discharge(CapButton_TypeDef CapBut)
{
GPIO_InitTypeDef GPIO_InitStructure;
  ...
  GPIO_InitStructure.GPIO_Pin = CAP_BUTTON_PIN[CapBut];
  GPIO_Init(CAP_BUTTON_PORT[CapBut], &GPIO_InitStructure);
  GPIO_ResetBits(CAP_BUTTON_PORT[CapBut], CAP_BUTTON_PIN[CapBut]);
}

/*************************************************************************
 * Function Name: STM_CapBut_Prepare
 * Parameters: CapButton_TypeDef CapBut
 *
 * Return: none
 *
 * Description: Prepares the given capacitive sensor button for measurement.
 *
 *************************************************************************/
void STM_CapBut_Prepare(CapButton_TypeDef CapBut)
{
GPIO_InitTypeDef GPIO_InitStructure;
  ...
  {
    case CAP_BUT1:
      /* Configure Cap2 Button pin as output */
      GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
      GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
      GPIO_InitStructure.GPIO_Pin = CAP_BUT2_PIN;
      GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
      GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
      GPIO_Init(CAP_BUT2_GPIO_PORT, &GPIO_InitStructure);
      break;
      
    case CAP_BUT2:
      /* Configure Cap1 Button pin as output */
      GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
      GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
      GPIO_InitStructure.GPIO_Pin = CAP_BUT1_PIN;
      GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
      GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
      GPIO_Init(CAP_BUT1_GPIO_PORT, &GPIO_InitStructure);
      break;
  }
}

/*************************************************************************
 * Function Name: STM_CapBut_Charge
 * Parameters: CapButton_TypeDef CapBut
 *
 * Return: none
 *
 * Description: Charges the given capacitive sensor button.
 *
 *************************************************************************/
void STM_CapBut_Charge(CapButton_TypeDef CapBut)
{
  switch(CapBut)
  {
    case CAP_BUT1:
      GPIO_SetBits(CAP_BUT2_GPIO_PORT, CAP_BUT2_PIN);
      break;
    case CAP_BUT2:
      GPIO_SetBits(CAP_BUT1_GPIO_PORT, CAP_BUT1_PIN);
      break;
  }
}

/*************************************************************************
 * Function Name: STM_CapBut_Setup
 * Parameters: CapButton_TypeDef CapBut
 *
 * Return: none
 *
 * Description: Initialize the given capacitive sensor button.
 *
 *************************************************************************/
void STM_CapBut_Setup(CapButton_TypeDef CapBut)
{
EXTI_InitTypeDef EXTI_InitStructure;
NVIC_InitTypeDef NVIC_InitStructure;

  /* Enable Cap Button clock */
  RCC_AHB1PeriphClockCmd(CAP_BUTTON_CLK[CapBut], ENABLE);
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_SYSCFG, ENABLE);
  
  /* Connect Cap Button EXTI Line to Button GPIO Pin */
  SYSCFG_EXTILineConfig(CAP_BUTTON_PORT_SOURCE[CapBut], CAP_BUTTON_PIN_SOURCE[CapBut]);

  /* Configure Cap Button EXTI line */
  EXTI_InitStructure.EXTI_Line = CAP_BUTTON_EXTI_LINE[CapBut];
  EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
  EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Rising;
  EXTI_InitStructure.EXTI_LineCmd = ENABLE;
  EXTI_Init(&EXTI_InitStructure);
  
  /* Enable and set Cap Button EXTI Interrupt to the lowest priority */
  NVIC_InitStructure.NVIC_IRQChannel = CAP_BUTTON_IRQn[CapBut];
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0x0F;
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x0F;
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
  NVIC_Init(&NVIC_InitStructure);
