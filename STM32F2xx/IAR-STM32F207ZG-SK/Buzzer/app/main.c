 ...
 * to develop code for the IAR-STM32F207ZG-SK board. It shows basic use of the I/O,
 * the timer, the interrupt controllers and the LDC module
 *
 *    Press TAMPER button and the onboard buzzer will make a sound.
 *
...
int pwm_state = 0;
  ...
  /* TAMPER button init */
  STM_ButtonInit(BUTTON_TAMPER,BUTTON_MODE_GPIO);
  ...
  while(1)
  {
    if (Bit_RESET == STM_ButtonGetState(BUTTON_TAMPER))
    {
      if(0 == pwm_state)
      {
        pwm_state = 1;
