...
void RTC_IRQHandler(void)
{
  ...
  {
    /* Toggle the led connected to P2.4 pin each 1s */
    GPIO_WriteBit(GPIO2, GPIO_Pin_4, (BitAction)(1-GPIO_ReadBit(GPIO2, GPIO_Pin_4)));
    ...
    if(SecondCounter == 4)
    {
      /* Alarm in ~5s (as the period of the LPOSC shifts around +30% in STANDBY 
         mode, the Alarm period will be set to 3s) */
      ...
      /* Enter in STANDBY mode, the RTC Alarm event will wake-up the system in ~5s  */
