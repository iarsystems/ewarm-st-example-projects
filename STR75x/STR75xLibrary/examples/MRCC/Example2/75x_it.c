...
{
  /* If the wake-up from STOP mode is caused by RTC Alarm, clear Alarm pending bit */
...
void MRCC_IRQHandler(void)
{
  ...
  { /* The system will serve indefinitely this ISR as long as the OSC4M clock 
       is not detected (The user can perform here some rescue operations) */
...
void RTC_IRQHandler(void)
{
  if(RTC_GetITStatus(RTC_IT_Second) != RESET)
  {
    ...
    if(nCount == 4)
    {
      EnterSTOPMode = 1; /* Enable the system to enter STOP mode */
      nCount = 0;
    }
    else
    {
