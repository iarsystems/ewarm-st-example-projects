...
pSpc3RequestSenseResponse_t pRequestSenseResponse;
  if ((Cbw.bmCBWFlags.Dir == BotDataFormDevToHost) &&
      (Cbw.dCBWDataTransferLength >= pRequestSense->AllocationLength))
  {
    ...
    if ((pRequestSense->Control.Control == 0) && !pRequestSense->DESC)
    {
      ...
      BotSendDataInit((Int8U *)pRequestSenseResponse,
                      MIN(pRequestSense->AllocationLength,sizeof(Spc3RequestSenseResponse_t)),
                      TRUE);
      return;
    }
    else
...
pSpc3ModeSelect6_t pModeSelect = (pSpc3ModeSelect6_t)Cbw.CBWCB;
  if ((Cbw.bmCBWFlags.Dir == BotDataFormHostToDev) &&
      (Cbw.dCBWDataTransferLength >= pModeSelect->ParameterListLength))
  {
    ScsiCmdStatus(ScsiInvalidCbd);
  }
  else
...
pSpc3ModeSense6_t pModeSense = (pSpc3ModeSense6_t)Cbw.CBWCB;
  if ((Cbw.bmCBWFlags.Dir == BotDataFormDevToHost) &&
      (Cbw.dCBWDataTransferLength >= pModeSense->AllocationLength))
  {
    if (   ((pModeSense->PageCode    == 0x3F) 
         || (pModeSense->PageCode    == 0x00))
        && ( pModeSense->PC          == 0) 
...
Int32U AllocationLength =
  ((Int32U)pInquiry->AllocationLength[0]<<8) + pInquiry->AllocationLength[1];
  if ((Cbw.bmCBWFlags.Dir == BotDataFormDevToHost) &&
      (Cbw.dCBWDataTransferLength >= AllocationLength))
  {
...
Int32U AllocationLength = (pFormatCapacity->AllocationLength[0]<<8) +\
                           pFormatCapacity->AllocationLength[1];
