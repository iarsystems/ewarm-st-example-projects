...
uint8_t WavePlayer_Start(void)
{
  ...
  WaveFileStatus = WavePlayer_WaveParsing("STFILES", "STFILES/WAVE.WAV", &wavelen);
...
static ErrorCode WavePlayer_WaveParsing(uint8_t *DirName, uint8_t *FileName, uint32_t *FileLen)
{
  ...
  {
    *FileLen = fiwave.filelen;
    number_of_clusters = fiwave.filelen / 512;
    if ((fiwave.filelen % SECTOR_SIZE) > 0)
    {
      number_of_clusters ++;
    }
  }
