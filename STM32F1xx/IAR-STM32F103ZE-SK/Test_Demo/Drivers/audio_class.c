...
volatile ps16 pSpkData;
volatile u32 Delta,SampCount;
volatile s32 SampAdapt;
...
__no_init s16 AudioSpkData[SampRerFrame * 3 * 2];

#pragma data_alignment=4
u8 AudioBuf[2];

u8   AudioRequest,AudioCS,AudioCN,AudioId;
u16  AudioDataSize;
...
I2S_InitTypeDef I2S_InitStructure;
  ...
  /*RCC_APB2PeriphResetCmd(RCC_APB2Periph_GPIOB | RCC_APB2Periph_GPIOC, DISABLE);
  ...
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = USB_INTR_AUDIO_SAMP_PRIORITY;
...
void AudioInHadler (USB_Endpoint_t EP)
{
  ...
  {
    pSpkData += EpCnfg[EP].Size/sizeof(u16);
    if(pSpkData >= AudioSpkData + sizeof(AudioSpkData)/sizeof(u16))
    {
      pSpkData = AudioSpkData;
    }
    ...
    {
      if(pSpkData >= (AudioSpkData + (2*sizeof(AudioSpkData))/(3*sizeof(u16))))
      {
        // Enable output
        SampCount = 0;
        SampEna   = TRUE;
  ...
  USB_IO_Data(SpkEp,(pu8)pSpkData,SpkEpMaxSize,(void*)AudioInHadler);
}

/*************************************************************************
 * Function Name: I2S2_Handler
 * Parameters: none
 *
 * Return: none
 *
 * Description: I2S 2 interrupt handler
 *		
 *************************************************************************/
...
static Boolean SecSampl;
  ...
  if (SampEna)
  {
    if(Delta > 1)
    {
      ...
      if(SampCount >= sizeof(AudioSpkData)/sizeof(s16))
      {
        SampCount = 0;
      }
 ...
 *************************************************************************/
static
Boolean AudioFeatureGetReg (u32 CS, u32 Id)
{
  switch (CS)
  {
...
void UsbClassAudioData (USB_Endpoint_t EP)
{
  ...
  if(AudioId == FeatUnit1Id)
  {
    ...
    case FU_VOLUME_CONTROL:
      ...
      if (AudioId == FeatUnit1Id)
      {
        AudioFeat1Vol = AudioBuf[0] + ((u16)AudioBuf[1]<<8);
        if((u16)AudioFeat1Vol == MinVol)
        {
          AudioSpkVolMul = 0;
        }
        else
        {
          if (AudioFeat1Vol < (s16)Feat1MinVol)
          {
            AudioFeat1Vol = Feat1MinVol;
          }
          else if(AudioFeat1Vol > (s16)Feat1MaxVol)
          {
