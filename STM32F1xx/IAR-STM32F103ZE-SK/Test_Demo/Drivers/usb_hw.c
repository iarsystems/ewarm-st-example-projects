...
__root __no_init u32 USB_PacketMemory[256];
...
static const UsbStandardEpDescriptor_t USB_CtrlEpDescr0 =
{
  ...
  {(u8)UsbEpTransferControl | (u8)UsbEpSynchNoSynchronization | (u8)UsbEpUsageData},
  Ep0MaxSize,
  0
};
 ...
 * Parameters: volatile pu32 pReg, u32 Data
 *
 * Return: none
 *
 ...
 *
 *************************************************************************/
void EpCtrlSet_EA(volatile pu32 pReg, u32 Data)
{
  Data &= 0xF;
  Data |= *pReg & 0x0700 ;
 ...
 *************************************************************************/
inline
u16 EpCtrlGet_EA(volatile pu32 pReg)
{
  return(*pReg & 0x0F);
}
 ...
 * Parameters: volatile pu32 pReg, EpState_t Data
 *
 * Return: none
 *
 ...
 *************************************************************************/
inline
u32 EpCtrlGet_STAT_TX (volatile pu32 pReg)
{
u32 Data = *pReg;
 ...
 *
 *************************************************************************/
void EpCtrlSet_DTOG_TX(volatile pu32 pReg, u32 Data)
{
 ...
 *
 *************************************************************************/
void EpCtrlToggle_DTOG_TX(volatile pu32 pReg)
{
u32 Data = *pReg | (1UL << 6);
  Data &= ~0x7030;   // Clear the toggle bits without DTOG_TX (6)
  Data |= USB_CTRL_CLEAR_ONLY_MASK;
 ...
 *************************************************************************/
inline
u32 EpCtrlGet_DTOG_TX(volatile pu32 pReg)
{
 ...
 *
 *************************************************************************/
void EpCtrlClr_CTR_TX(volatile pu32 pReg)
{
u32 Data = *pReg;
 ...
 *************************************************************************/
inline
u32 EpCtrlGet_CTR_TX(volatile pu32 pReg)
{
 ...
 *
 *************************************************************************/
void EpCtrlSet_EP_KIND (volatile pu32 pReg, u32 Data)
{
  Data  = Data ? (1UL << 8):0;
 ...
 *************************************************************************/
inline
u32 EpCtrlGet_EP_KIND(volatile pu32 pReg)
{
 ...
 * Parameters: volatile pu32 pReg, EpType_t Data
 *
 * Return: none
 *
 ...
 *
 *************************************************************************/
void EpCtrlSet_EP_TYPE(volatile pu32 pReg, EpType_t Data)
{
u32 Type;
  Type = Data;
  Type <<= 9;
 ...
 *************************************************************************/
inline
u32 EpCtrlGet_EP_TYPE(volatile pu32 pReg)
{
u32 Data = *pReg;
  Data &= (3UL<<9);
  return(Data >> 9);
 ...
 *************************************************************************/
inline
u32 EpCtrlGet_SETUP(volatile pu32 pReg)
{
u32 Data = *pReg & (0x1UL << 11);
  return(Data?1:0);
}
 ...
 *
 *************************************************************************/
void EpCtrlSet_STAT_RX (volatile pu32 pReg, EpState_t Data)
{
u32 State;
  // The EP can change state between read and write
  // operations from VALID to NAK and result of
  // set operation will be invalid!
  if(EpCtrlGet_STAT_RX(pReg) != (u32)Data)
  {
    State = Data;
    State <<= 12;
    State  ^= *pReg;
 ...
 *************************************************************************/
inline
u32 EpCtrlGet_STAT_RX(volatile pu32 pReg)
{
u32 Data = *pReg &( 0x3UL << 12);
  return(Data >> 12);
}
 ...
 *
 *************************************************************************/
void EpCtrlSet_DTOG_RX(volatile pu32 pReg, u32 Data)
{
 ...
 *
 *************************************************************************/
void EpCtrlToggle_DTOG_RX(volatile pu32 pReg)
{
u32 Data = *pReg | (1UL << 14);
 ...
 *************************************************************************/
inline
u32 EpCtrlGet_DTOG_RX(volatile pu32 pReg)
{
 ...
 *
 *************************************************************************/
void EpCtrlClr_CTR_RX(volatile pu32 pReg)
{
u32 Data = *pReg;
 ...
 *************************************************************************/
inline
u32 EpCtrlGet_CTR_RX(volatile pu32 pReg)
{
...
#elif defined(IAR_STM32F103ZE_SK)
  // GPIO assign to the USB engine
 /* RCC_APB2PeriphResetCmd(  RCC_APB2Periph_GPIOA
 ...
 * Parameters: pu16 pOffset,u32 PacketSize,
 ...
 *************************************************************************/
static
Boolean USB_AllocateBuffer (pu16 pOffset,pu32 pPacketSize,
                            USB_Endpoint_t EndPoint)
{
pPacketMemUse_t  pPacketMem = pPacketMemUse, pPacketMemNext, pPacketMemUseNew;
//u32 MaxPacketSize = *pPacketSize+4;  // for CRC
u32 MaxPacketSize = *pPacketSize;
  if(MaxPacketSize > 62)
  {
  ...
  *pOffset = 0;
  ...
  if((*pOffset + MaxPacketSize) >=
     (((u32)__segment_size("USB_PACKET_MEMORY") -
...
USB_Endpoint_t EP;
pEpCnfg_t pEP;
volatile pu32 pEpCtrlBaseAddr;
u16 Offset,Offset1;
u32 MaxPacketSizeTmp;
 ...
 * Parameters: u32 DevAdd - device address between 0 - 127
 *
 * Return: none
 *
...
#elif defined(IAR_STM32F103ZE_SK)
  GPIO_WriteBit(GPIOG,GPIO_Pin_11,((Conn)?Bit_RESET:Bit_SET));
#endif
}
...
pEpCnfg_t pEP = &EpCnfg[EP];
  ...
  if(EP & 1)
  {
    ...
    while(pEP->AvbBuff)
    {
      ...
      case UsbEpTransferControl:
      case UsbEpTransferInterrupt:
        pDst = (pu32)__Offset2Addr_PackMem(ReadEpDTB_AddrTx(pEP->EpSlot));
        break;
      case UsbEpTransferBulk:
        if (!pEP->bDoubleBuffered)
        {
        ...
        else
        {
          ...
          {
            // Rx Buffer
            pDst = (pu32)__Offset2Addr_PackMem(ReadEpDTB_AddrRx(pEP->EpSlot));
          }
        }
        break;
      case UsbEpTransferIsochronous:
  ...
  else
  {
    ...
    while(pEP->AvbBuff)
    {
      ...
      case UsbEpTransferInterrupt:
        ...
        pSrc = (pu32)__Offset2Addr_PackMem(ReadEpDTB_AddrRx(pEP->EpSlot));
        break;
      case UsbEpTransferBulk:
        if (!pEP->bDoubleBuffered)
        {
        ...
        else
        {
          if (!(*pEP->pEpCtrl & (1UL << 6)))  // DTOGTX
          {
            ...
            pSrc = (pu32)__Offset2Addr_PackMem(ReadEpDTB_AddrTx(pEP->EpSlot));
          }
          else
          {
            // Rx Buffer
 ...
 *
 *************************************************************************/
USB_Endpoint_t USB_EpLogToPhysAdd (u8 EpLogAdd)
{
    ...
    if(!EpCnfg[EP].bDoubleBuffered)
    {
      ...
      if(EP == CTRL_ENP_OUT)
      {
        ...
        if (SetupPacket)
        {
          ...
          USB_IO_Data(CTRL_ENP_IN,NULL,(u32)-1,NULL);
