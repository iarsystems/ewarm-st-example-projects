...
static u32 XL_Win = DEF_X_LEFT;
static u32 YU_Win = DEF_Y_UP;
static u32 XR_Win = DEF_X_RIGHT;
static u32 YD_Win = DEF_Y_DOWN;

static u32 TabSize = TEXT_DEF_TAB_SIZE;


/*******************************************************************************
* Function Name  : GLCD_SendCmd
* Description    : Send command to the Graphic LCD                  
* Input          : GLCD_SendCmd Cmd, pu8 pData, u32 Size
* Output         : None
...
static u32 ResRatio;             // Hold resistor ratio init by VOLCTR
  ...
  if(Write2_DRAM)
  {
    ...
    else
    {
      ...
      while(Size > 1)
      {
        ...
        else
        {
          ...
          GLCD_SPI_SendBlock((pu8)&PixelHold,3);
          PixelHold = 0;
        }
	...
	if(Glcd_Iss==GLCD_ISS_0)
	{
      ...
      DataMode.MY = pDATCTR_Data->MY;
			...
			GLCD_SendCmd(MADCTR,(pu8)&DataMode,0);
      // 4K colors Type A
			ColourSch = 3;			
    ...
    case SETCON:
      ...
      return(GLCD_SendCmd(VOLCTR,(pu8)&Contrast,0));
    case VOLCTR:
      ResRatio = *(pData+1);
      break;
		}
...
pDATCTR_Data_t pDATCTR_Data;
  ...
  if(Glcd_Iss == GLCD_ISS_1)
  {
    ...
    assert(GLCD_DEF_DPL_ML == 0);
	  ...
	  GLCD_SendCmd(DISCTR,(pu8)&Data,0);
	  //  OSCON (Oscillator ON)
	  GLCD_SendCmd(OSCON,NULL,0);
    // Set contrast
    Data = 0x20 | (0x0UL << 8);
    GLCD_SendCmd(VOLCTR,(pu8)&Data,0);
  }
  else
  {
    // Set contrast
    Data = 0x30;
    GLCD_SendCmd(SETCON,(pu8)&Data,0);
  }
...
#error "PARTIAL_MODE not supported yet"
  ...
  if(Glcd_Iss == GLCD_ISS_1)
  {
    // PWRCTR
    Data = 0x1F;
    GLCD_SendCmd(PWRCTR,(pu8)&Data,0);
  }
                             ...
                             pu32 pH_Size, pu32 pV_Size)
{
  ...
  *pXL = XL_Win + (x*pCurrFont->H_Size);
  if(*pXL > XR_Win)
  {
    return(FALSE);
  }
  *pYU = YU_Win + (y*pCurrFont->V_Size);
  if(*pYU > YD_Win)
  {
    return(FALSE);
  }

  *pXR   = XL_Win + ((x+1)*pCurrFont->H_Size) - 1;
  if(*pXR > XR_Win)
  {
  	*pH_Size -= *pXR - XR_Win;
    *pXR = XR_Win;
  ...
  *pYD = YU_Win + ((y+1)*pCurrFont->V_Size) - 1;
  if(*pYD > YD_Win)
  {
    *pV_Size -= *pYD - YD_Win;
    *pYD = YD_Win;
...
pu8 pSrc;
u32 H_Line;
u32 xl,xr,yu,yd,Temp,V_Size, H_Size, SrcInc = 1;
u32 WhiteSpaceNumb;
  if(pCurrFont == NULL)
  {
    return(EOF);
  ...
  case '\t':  // go to next Horizontal Tab stop
  	WhiteSpaceNumb = TabSize - (x%TabSize);
  	for(u32 k = 0; k < WhiteSpaceNumb; ++k)
  	{
      LCD_SET_WINDOW(xl,xr,yu,yd);
	  	if(GLCD_TextCalcWindow(&xl,&xr,&yu,&yd,&H_Size,&V_Size))
	  	{
