...
pUsbStandardInterfaceDescriptor_t pInterface;
  ...
  {
    // deconfigure device
    for (u32 i = 0; i < UsbDevCtrl.Interfaces; ++i)
    {
...
void USB_StatusEndHandler (USB_Endpoint_t EP)
{
  ...
  USB_IO_Data(CTRL_ENP_OUT,NULL,(u32)-1,NULL);
}
...
pUsbStandardInterfaceDescriptor_t pData;
  ...
  for( pData = (UsbStandardInterfaceDescriptor_t *)pConfiguration;
      (u8*)pData < (u8 *)pConfiguration + pConfiguration->wTotalLength;
       pData =(UsbStandardInterfaceDescriptor_t *)((u8 *)pData + pData->bLength))
  {
...
{
#if USB_HIGH_SPEED > 0
const u8 ** ppCfgArray = (const u8**)UsbDescArray[(UsbDevCtrl.HighSpeed?USB_HS_CFG_ARRAY:USB_FS_CFG_ARRAY)];
#else
const u8 ** ppCfgArray = (const u8**)UsbDescArray[USB_FS_CFG_ARRAY];
#endif
...
static u32 ep_numbers;
  assert(pInterface != NULL);
  if (pEp == NULL)
  {
    ep_numbers = pInterface->bNumEndpoints;
    pEpNext = (pUsbStandardEpDescriptor_t)(((pu8)pInterface)+pInterface->bLength);
  }
  else
  {
    pEpNext = (pUsbStandardEpDescriptor_t)(((pu8)pEp) + pEp->bLength);
  }
  // Find Interface
  for(;ep_numbers != 0 && pEpNext->bLength;
       pEpNext = (pUsbStandardEpDescriptor_t)(((pu8)pEpNext) + pEpNext->bLength))
  {
...
UsbCommStatus_t UsbDevStatus(u16 Index)
{
  if(Index == 0)
  {
    InData = ((u32)UsbDevCtrl.Feature.Data) & 0xFF;
    USB_IO_Data(CTRL_ENP_IN,(pu8)&InData,2,(void*)USB_StatusHandler);
    return(UsbPass);
  }
  return(UsbFault);
}
...
UsbCommStatus_t UsbGetDescriptorDevice(u8 Index)
{
  if(Index == 0)
  {
    USB_IO_Data(CTRL_ENP_IN,
               (u8 *)&UsbStandardDeviceDescriptorStr,
...
#if USB_HIGH_SPEED > 0
  pCfg = (pUsbStandardConfigurationDescriptor_t)(*(u8 **)UsbDescArray[(UsbDevCtrl.HighSpeed?USB_HS_CFG_ARRAY:USB_FS_CFG_ARRAY)] + Index);
#else
  pCfg = (pUsbStandardConfigurationDescriptor_t)(*(u8 **)UsbDescArray[USB_FS_CFG_ARRAY] + Index);
#endif // USB_HIGH_SPEED > 0
  USB_IO_Data(CTRL_ENP_IN,
             (pu8)pCfg,
...
u8 **ppData;
#if USB_HIGH_SPEED > 0
pu8 pLangStr = (pu8)UsbDescArray[(UsbDevCtrl.HighSpeed?USB_HS_LANG_STR:USB_FS_LANG_STR)];
#else
pu8 pLangStr = (pu8)UsbDescArray[USB_FS_LANG_STR];
#endif // USB_HIGH_SPEED > 0
u32 i;
  if(Index == 0)
  {
  ...
  else
  {
    ...
    // Find String index
  #if USB_HIGH_SPEED > 0
    for(ppData = (*((u8***)UsbDescArray[(UsbDevCtrl.HighSpeed?USB_HS_STR_ARRAY:USB_FS_STR_ARRAY)] + LanguagesOffset));
  #else
    for(ppData = (*((u8***)UsbDescArray[USB_FS_STR_ARRAY] + LanguagesOffset));
  #endif // USB_HIGH_SPEED > 0
        ppData ;
        ++ppData)
    {
 ...
 *************************************************************************/
inline static
UsbCommStatus_t UsbGetQualifierDescriptor(u8 Index)
{
  USB_IO_Data(CTRL_ENP_IN,
             (u8 *)&UsbQualifierDescriptor,
 ...
 *************************************************************************/
inline static
UsbCommStatus_t UsbGetOtherSpeedDescriptorConfiguration(u8 Index)
{
pUsbStandardConfigurationDescriptor_t pCfg;
  if (Index >= UsbStandardDeviceDescriptorStr.bNumConfigurations)
  {
  ...
  pCfg = (pUsbStandardConfigurationDescriptor_t)(*(u8 **)UsbDescArray[(UsbDevCtrl.HighSpeed?USB_HS_OTHER_CFG_ARRAY:USB_FS_OTHER_CFG_ARRAY)] + Index);
  USB_IO_Data(CTRL_ENP_IN,
             (pu8)pCfg,
              USB_T9_Size(pCfg->wTotalLength,UsbEp0SetupPacket.wLength.Word),
             (void*)USB_StatusHandler);
...
void UsbGetConfiguration(void)
{
     ...
     (UsbEp0SetupPacket.wLength.Word == 1))
  {
    ...
    USB_IO_Data(CTRL_ENP_IN,(pu8)&InData,1,(void*)USB_StatusHandler);
  }
  else
  {
    USB_T9_ERROR_REQUEST();
...
void UsbGetInterface (void)
{
  ...
  {
    USB_IO_Data(CTRL_ENP_IN,
               (pu8)&UsbDevCtrl.AlternateSetting[UsbEp0SetupPacket.wIndex.Word],
                1,
               (void*)USB_StatusHandler);
  }
  else
