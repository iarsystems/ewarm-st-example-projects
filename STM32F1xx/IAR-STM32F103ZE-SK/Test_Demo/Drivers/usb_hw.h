...
#define USB_OffsetOfDBT (((u32)__segment_size("USB_PACKET_MEMORY")/2) - (sizeof(USB_BuffDeskTbl_t) * (MaxIndOfRealizeEp+1)) & ~7)

#define __Offset2Addr_PackMem(Offset) ((u32)__segment_begin("USB_PACKET_MEMORY") + ((Offset<<1) & ~3) + (Offset&1))

#define ReadEpDTB(Slot,Offset)  (*((pu16)(__Offset2Addr_PackMem((USB_OffsetOfDBT + (Slot)*sizeof(USB_BuffDeskTbl_t)  + (Offset))))))
...
typedef struct _EpCnfg_t
{
  ...
  union
  {
    ...
    struct
    {
      u8 bDoubleBuffered     : 1;
      u8 bZeroPacket         : 1;
      u8 bZeroPacketPossible : 1;
    };
  };
} EpCnfg_t, *pEpCnfg_t;
