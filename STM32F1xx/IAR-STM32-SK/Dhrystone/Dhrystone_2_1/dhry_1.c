...
extern unsigned long GetTimersTick();
 ...
 ***********************************************************************/
   ...
   do
     {
       ...
       printf ("%12.0f runs %6.2f seconds \n",(double) Number_Of_Runs, ((double)User_Time)/(TICK_PER_MSEC*1000.0));
       if (User_Time > 5000 * TICK_PER_MSEC)
         {
             count = 0;
         }
       else
         {
             if (User_Time < (100 * TICK_PER_MSEC))
               {
   ...
   {
     Microseconds = ((double)User_Time)/(TICK_PER_MSEC*1000.0) * Mic_secs_Per_Second
                         / (double) Number_Of_Runs;
     Dhrystones_Per_Second = (double) Number_Of_Runs / (((double)User_Time)/((double)TICK_PER_MSEC*1000.0));
