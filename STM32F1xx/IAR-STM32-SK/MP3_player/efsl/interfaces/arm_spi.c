 ...
 *
 *    File name   : arm_spi.c
 *    Description : link between efs low level functions
 *
 *    History :
...
{
  if(SdDiskIO(buf,address,1,DiskRead) == DiskCommandPass)
  {
    return(0);
  }
  return(1);
...
{
  if(SdDiskIO(buf,address,1,DiskWrite) == DiskCommandPass)
  {
    return(0);
  }
  return(1);
