...
int main(void)
{
  ...
  /* ---------------------------------------------------------------------------
      Using an external 4 MHz to generate a 32 MHz frequency
    --------------------------------------------------------------------------- */


  /*  Configure the PLL1 ( * 16 , / 1 ) */
  ...
  /*  Timer 1 Input Capture A function pin (P1.4) configured as Input
      Tristate TTL. */
  ...
  while(1)
   {
      ...
      while (TIM_FlagStatus(TIM1,TIM_ICFB) == RESET);
      Tmp1 = TIM1->ICBR;

      /* wait until ICFA flag is set  */
      while (TIM_FlagStatus(TIM1,TIM_ICFA) == RESET);
      Tmp2 = TIM1->ICAR;
      ...
      /*  Clear TIMER Flags*/
      TIM_FlagClear(TIM1,TIM_ICFA);
      TIM_FlagClear(TIM1,TIM_ICFB);
