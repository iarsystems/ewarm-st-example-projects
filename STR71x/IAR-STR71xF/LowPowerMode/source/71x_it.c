...
* Date First Issued  : DECEMEBER 2004
* Description        : Interrupt Service Routines for the STR71x Power
*                      Management AN2100
*********************************************************************************
* History:
* 24/05/05 : V3.0
...
{
    u32 i;
    for(i=0; i<100; i++);                               //some delay for the flash stabilization ( for LPRWFI mode)
    RTC_FlagClear ( RTC_AIR );				//clear the pending interrupt flags
    RTC_FlagClear ( RTC_GIR );
    RTC_ITConfig(RTC_GIT,DISABLE);                      //disable RTC interrupts
...
{
  XTI->PRH=0;                                 //clear pending XTI interrupts
  XTI->PRL=0;
}
