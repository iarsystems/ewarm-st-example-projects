...
{
	return (PCU->PWRCTRL & PCU_VROK_Mask) == 0x00 ? PCU_UNSTABLE : PCU_STABLE;
}

/*******************************************************************************
* Function Name  : PCU_FlagSatus
* Description    : This routine is used to return the PCU register flag
* Input 1        : The flag to get
* Return         : RESET, SET
*******************************************************************************/
inline functionalstate PCU_FlagSatus ( PCU_Flags Xflag )
{
	return ( PCU->PWRCTRL & Xflag ) == 0x00 ? RESET : SET;
}
...
{
	return ( PCU->PWRCTRL & xVR ) == 0  ? ENABLE : DISABLE;
}

/*******************************************************************************
* Function Name  : PCU_LVDEnable
* Description    : This routine is used to enable the Low Voltage Regulator.
* Input          : None
* Return         : None
*******************************************************************************/
inline void PCU_LVDEnable ( void )
{
	PCU->PWRCTRL |= PCU_WREN_Mask;	// Unlock Power Control Register
	PCU->PWRCTRL &= ~PCU_LVD_Mask;	// Reset LVD Flag
}

/*******************************************************************************
* Function Name  : PCU_LVDStatus
* Description    : This routine is used to get the LVD Status.
* Input          : None
* Return         : ENABLE, DISABLE
*******************************************************************************/
inline functionalstate PCU_LVDStatus ( void )
{
	return ( PCU->PWRCTRL & PCU_LVD_Mask ) == 0 ? ENABLE : DISABLE;
}
